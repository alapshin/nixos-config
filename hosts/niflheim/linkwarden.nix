  ...
  services = {
    linkwarden = {
      ...
      environment = {
        ...
        AUTHELIA_WELLKNOWN_URL = "https://${config.domain.auth}/.well-known/openid-configuration";
      };
      secretsFile = config.sops.templates."linkwarden.env".path;
    };

    nginx-ext.applications."linkwarden" = {
      auth = false;
          ...
          clients = [
            {
              ...
              redirect_uris = [
                "https://linkwarden.${config.domain.base}/api/v1/auth/callback/authelia"
              ];
            }
          ];
        };
